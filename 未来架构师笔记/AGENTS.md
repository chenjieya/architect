...
- **图片处理边界**：图片是理解型笔记的一部分，Ingest 源文件含图片时，AI 整理 Wiki 必须结合图片与正文组织内容，不得默认省略图片。
  ...
  - 本地图片：若图片文件在本机真实存在且可读取，复制到目标 Wiki 笔记同目录，用 `![[图片名.png]]` 嵌入；AI 不主动上传到图床。
  - 失效图片：若本地路径不存在或无法读取，在回复的图片清单中注明“原图缺失/无法读取”。
  - 完成后必须在回复末尾列出「涉及图片的文件清单」，包含远程图片 URL、本地已复制图片、缺失图片，方便用户自行用图床插件上传或替换。
...
5. **源文件含图片时，整理 Wiki 必须图文结合**：
   ...
   - 本机可读取图片 → 复制到目标 Wiki 笔记同目录，用 `![[图片名.png]]` 嵌入，必要时配简短中文图注。
   - 本机缺失/不可读取图片 → 不臆造替代图，不删除引用事实，在回复图片清单中标明“原图缺失/无法读取”。
   - **完成后必须在回复末尾列出「涉及图片的文件清单」**：远程图片列 URL，本地图片列复制后的文件路径，缺失图片列原始路径与缺失状态，方便用户用 Obsidian 的 "Upload all images" 命令上传到图床或手动替换。
