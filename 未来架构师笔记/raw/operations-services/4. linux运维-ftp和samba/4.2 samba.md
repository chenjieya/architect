...
valid users = @t (访问权限为 t 组）

write list = @t (写权限为 pj 组）

备注：客户端连接时，显示的是 data 名，而非 username;修改配置可以先 testparm 下（检查配置文件）。
...
备注：smb 用户与系统上帐号的密码无关，修改系统密码不影响 smb 用户之前设置的密码.
...
客户端在挂载的时候报错的话，注意看下服务端和客户端是否都安装了 cifs-utils 包
