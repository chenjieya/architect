...
初始化Cesium地图/地球：下载模块，设置资源，设置token

Cesium底图：默认bing map，可以选择：天地图、高德、百度等

Cesium控件：显示隐藏，位置与样式，自定义交互逻辑等
...
Controller控制器：scene.screenSpaceCameraController，缩放，拖拽，倾斜等
...
3D模型：glTF格式（.glb），viewer.entities.add({model})
...
与DOM配合：点击实体弹出DOM提示框等

钩子：tileLoadProgressEvent，postRender等
...
kml：谷歌的一种类xml的格式，Cesium.KmlDataSource.load

CZML： Cesium专用的一种数据格式，Cesium.CzmlDataSource.load
...
默认自带的tiles数据：Cesium OSM Buildings，Google Photorealistic 3D Tiles ...

模型等转tiles格式：glTF，CityGML ...

本地加载tiles数据：tileset.json

如何生成tiles数据：cesium ion平台，cesiumlab平台
...
+ clock
+ 方向
+ 路线：高德API，geojson，CZML

clock： 决定Cesium世界的时间流逝，viewer.clock，viewer.timeline

方向：改变模型的方向，1变换矩阵，2模型编辑器

路线：经纬度 + 高度的坐标点，1高德API，2geojson，3CZML
...
概念：是运行在GPU上的小程序， 顶点着色器： 把3D顶点转换到屏幕坐标， 片段着色器： 决定每个像素的最终颜色和透明度，vertexShaderSource/fragmentShaderText
