...
### 9.1 一个典型的 Python + UV 项目的 Makefile
...
1. 将`27. 异步编程`的代码改造成 UV 工程的格式。
2. 将`29. 构建发布`的代码使用 uv 发布到阿里云私有仓库
