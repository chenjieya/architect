...
前面我们分别介绍了事件循环、Future 和协程的底层原理。接下来，我们不再自己造轮子，而是使用 Python 官方和社区给我们准备好的 API，高效地进行异步开发。
...
### 3.3 create_task —— 将协程包装成 Task 并调度执行
...
### 3.7 TaskGroup —— 结构化并发（Python 3.11+）
...
### 3.13 asyncio.timeout（Python 3.11+）
...
#### 4.1.1 aiohttp（第三方最流行的异步 HTTP 库）
...
#### 4.1.2 httpx（支持同步/异步双模式，API 更友好）
