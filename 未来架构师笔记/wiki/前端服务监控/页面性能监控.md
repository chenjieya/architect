...
![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154304588.png)

**Lighthouse 能够生成一份该网站的报告，比如下图：**

![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154314229.png)
...
### 1.1 Lighthouse 给出 Opportunities 优化建议

Lighthouse 会针对当前网站，给出一些`Opportunities`优化建议

**Opportunities 指的是优化机会，它提供了详细的建议和文档，来解释低分的原因，帮助我们具体进行实现和改进**

![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154325724.png)
...
### 1.2 Lighthouse 给出 Diagnostics 诊断问题列表

**`Diagnostics` 指的是现在存在的问题，为进一步改善性能的验证和调整给出了指导**

![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154337805.png)
...
## 2. Performance 寻找性能瓶颈

打开 Chrome 浏览器控制台，选择`Performance`选项，点击左侧`reload图标`

![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154355865.png)
...
![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154420078.png)
...
## 4. [PerformanceObserver](https://developer.mozilla.org/zh-CN/docs/Web/API/PerformanceObserver)
...
简单来说，我们只需要指定预定的[entryType](https://developer.mozilla.org/zh-CN/docs/Web/API/PerformanceEntry/entryType)，然后就能通过 PerformanceObserver 的回调函数获取相应的性能指标数值
...
w3c 制定了一大堆指标，不过 google 发布了`web-vitals` ，它是一个开源的用以衡量性能和用户体验的工具，对于我们现在来说，这个开源工具中所提到的指标已经足够用了。而且现在本身也是业界标准

## 5. [web-vitals](https://web.dev/metrics/)

### 5.1 [以用户为中心的性能指标](https://web.dev/user-centric-performance-metrics/)
...
### 5.2 [什么是 FCP？](https://web.dev/fcp/)

首次内容绘制 (FCP) 指标测量页面从开始加载到页面内容的任何部分在屏幕上完成渲染的时间。对于该指标，"内容"指的是文本、图像（包括背景图像）、`<svg>`元素或非白色的`<canvas>`元素。

![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154438359.png)
...
![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154448891.png)

#### 5.2.1 怎样算是良好的 FCP 分数？

为了提供良好的用户体验，网站应该努力将首次内容绘制控制在**1.8 秒**或以内。为了确保您能够在大部分用户的访问期间达成建议目标值，一个良好的测量阈值为页面加载的**第 75 个百分位数**，且该阈值同时适用于移动和桌面设备。

### 5.3 [什么是 LCP？](https://web.dev/lcp/)
...
![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154458734.png)
...
![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154510577.png)
...
![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154523227.png)
...
![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154548390.png)

在上图中，有一个元素在一帧中占据了一半的可视区域。接着，在下一帧中，元素下移了可视区域高度的 25%。红色虚线矩形框表示两帧中元素的可见区域集合，在本示例中，该集合占总可视区域的 75%，因此其*影响分数*为`0.75` 。

### 5.5 [什么是 FID？](https://web.dev/fid/)

FID 测量从用户第一次与页面交互（例如当他们单击链接、点按按钮或使用由 JavaScript 驱动的自定义控件）直到浏览器对交互作出响应，并实际能够开始处理事件处理程序所经过的时间。

![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154558333.png)

#### 5.5.1 如果交互没有事件侦听器怎么办？
...
![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/20260205154700313.png)
...
| DomReady  | HTML 加载完成时间也就是 DOM Ready 时间。 | domContentLoadEventEnd - fetchStart | 单页面客户端渲染下，为生成模板 dom 树所花费时间；非单页面或单页面服务端渲染下，为生成实际 dom 树所花费时间' |
