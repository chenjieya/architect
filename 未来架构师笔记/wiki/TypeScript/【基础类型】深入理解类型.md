...
![image.png](https://picgo-1300696809.cos.ap-beijing.myqcloud.com/202405071945942.png)
...
这里只讲解接口的基本用法，接口其实是面向对象中的概念，而且接口和类型别名之间虽然 80%的情况可以互换使用，但是还是有很重要的区别，我们后面一起分析。
...
虽然有时候口头上经常会说交集类型，但我并不是太喜欢把`&`符号称为交集，叫做交叉应该更容易理解一些，不容易造成思想误区。
...
其实前面说过的`字面量的类型检查`，`typeof`，`instanceof`，`in`以及`自定义守卫`在 Typescript 中有统一的称呼，都叫做**类型守卫**，其目的其实都是在控制流分析的时候，帮助 typescript 收紧类型，便于推断
...
前面介绍过`null`，`undefined`和`void`类型，这三个都是有具体意义的，也表示具体的类型，`undefined`表示尚未定义，`null`表示缺少值，甚至是`void`就表示一个空类型，就像没有返回值的函数使用 void 来作为返回值类型标注一样。
...
#### 22.2.4 将 `any/unknown` 断言为一个具体的类型
