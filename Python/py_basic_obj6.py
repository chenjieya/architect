...
for item in students:
    pass
    # print(f"姓名： {item.get('name')}, 年龄：{item.get('age')}")
...
for item in students:
    if item.get("sex") == "女" and item.get("age") < 25:
        pass
        # young_females.append(item)
...
for item in students:
    if item.get("name")[0] == "陈":
        chen_students.append(item)
...
for item in students:
    if item.get("tel")[-1] == "1":
        pass
        # tel_end_with_1.append(item)
print(tel_end_with_1)
...
for item in students:
    if item.get("sex") == "女":
        pass
        # female_names.append(item.get("name"))
print(female_names)
...
for item in students:
    if item.get("sex") == "女":
        pass
        # female_contacts.append(dict(name=item.get("name"), tel=item.get("tel")))
...
while i < len(students):
    age = int(students[i].get("age"))
    count_age += age
    i += 1
else:
    avage_age = count_age / len(students)
print(f"总年龄：{count_age}, 平均年龄：{avage_age:.2f}")
...
result = dict(names=names, ages=ages)
print(result)
...
while i < len(students):
    item = students[i]
    if item.get("id") == 796997:
        pass
        # print(f"姓名： {item.get("name")}, 电话：{item.get("tel")}")
        break
    i += 1
else:
    print("未找到")
...
for item in students:
    if item.get("sex") == "女" and item.get("age") > 28:
        print(False)
        break
else:
    print(True)
