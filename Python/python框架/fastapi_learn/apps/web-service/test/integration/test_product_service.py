...
from apps.schema.category import CategoryCreate
from apps.schema.product import ProductCreate, ProductUpdate
from apps.service.category_service import CategoryService
from apps.service.product_service import ProductService
...
class TestListProducts:
    ...
    async def test_filter_by_keyword(self, db_session: AsyncSession):
        ...
        await svc.create_product(
            ProductCreate(name="苹果手机", description="高端智能手机")
        )
...
class TestUpdateProduct:
    ...
    async def test_update_categories(self, db_session: AsyncSession):
        ...
        result = await svc.update_product(
            created.id, ProductUpdate(category_ids=[cat.id])
        )
        assert result is not None
    ...
    async def test_update_clear_categories(self, db_session: AsyncSession):
        ...
        created = await svc.create_product(
            ProductCreate(name="商品", category_ids=[cat.id])
        )
