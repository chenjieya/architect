...
from apps.schema.product import SkuBrief
...
class TestSkuCreate:
    ...
    def test_missing_sku_code_fails(self):
        with pytest.raises(ValidationError):
            SkuCreate(
                price=Decimal("10"), attrs={}, image_url="http://x"
            )  # pyright: ignore[reportCallIssue]

    def test_missing_attrs_fails(self):
        with pytest.raises(ValidationError):
            SkuCreate(
                sku_code="S", price=Decimal("10"), image_url="http://x"
            )  # pyright: ignore[reportCallIssue]

    def test_missing_price_fails(self):
        with pytest.raises(ValidationError):
            SkuCreate(
                sku_code="S", attrs={}, image_url="http://x"
            )  # pyright: ignore[reportCallIssue]

    def test_empty_sku_code_fails(self):
        with pytest.raises(ValidationError):
            SkuCreate(
                sku_code="", price=Decimal("10"), attrs={}, image_url="http://x"
            )  # pyright: ignore[reportCallIssue]

    def test_sku_code_too_long_fails(self):
        with pytest.raises(ValidationError):
            SkuCreate(
                sku_code="x" * 51, price=Decimal("10"), attrs={}, image_url="http://x"
            )  # pyright: ignore[reportCallIssue]
