...
class TestProductCreate:
    ...
    def test_category_ids_with_non_int_fails(self):
        with pytest.raises(ValidationError):
            ProductCreate(
                name="iPhone", category_ids=["not_int"]
...
class TestProductBrief:
    ...
    def test_brand_can_be_none(self):
        ...
        p = ProductBrief(
            id=1, name="Test", description="desc", brand=None, created_at=now
        )
        assert p.brand is None
    ...
    def test_all_fields_present(self):
        now = datetime.now(timezone.utc)
        p = ProductBrief(
            id=42, name="Widget", description="A widget", brand="Acme", created_at=now
        )
        assert p.id == 42
