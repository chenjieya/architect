...
@pytest.fixture
def uploader(config):
    with patch("app_utils.upload.aliyun.oss2.Auth") as mock_auth, \
         patch("app_utils.upload.aliyun.oss2.Bucket") as mock_bucket:
...
class TestGenerateUploadCredentials:
    def test_successful_credential_generation(self, config):
        with patch("app_utils.upload.aliyun.oss2.Auth"), \
             patch("app_utils.upload.aliyun.oss2.Bucket"), \
             patch("app_utils.upload.aliyun.uuid.uuid4") as mock_uuid, \
             patch("app_utils.upload.aliyun.datetime") as mock_dt, \
             patch("app_utils.upload.dir_strategy.datetime") as mock_dir_dt:
...
class TestDelete:
    async def test_delete_calls_bucket_delete_object(self, uploader):
        with patch("app_utils.upload.aliyun.asyncio.to_thread") as mock_to_thread:
...
class TestExists:
    async def test_exists_true(self, uploader):
        with patch("app_utils.upload.aliyun.asyncio.to_thread", return_value=True):
            result = await uploader.exists("uploads/2025/06/26/test.png")
            assert result is True

    async def test_exists_false(self, uploader):
        with patch("app_utils.upload.aliyun.asyncio.to_thread", return_value=False):
            result = await uploader.exists("uploads/2025/06/26/test.png")
